...
/**
 * OOXML preset names whose `<a:pathLst>` declares
 * `stroke="true" fill="none"` — "lines and connectors". Their
 * generators emit open polylines/curves that should be stroked, never
 * filled (Canvas's `fill()` would auto-close the open subpath and
 * paint, e.g., a bent connector as a filled triangle).
 *
 * The `IPresetPath { d, fill, stroke }` metadata that exists for
 * `multiPathPresets` is not present in this single-path registry —
 * this set restores the missing fill metadata for the closed
 * ECMA-376 §20.1.10.55 (ST_ShapeType) family of connectors / lines.
 *
 * Note: `bracePair` / `bracketPair` / individual `*Brace` / `*Bracket`
 * presets are NOT in this set. They ARE fillable in Word — the visual
 * "outline only" appearance comes from the geometry tracing the
 * brace/bracket curves with no enclosed interior to fill, not from a
 * stroke-only path declaration. We instead fix their generators to
 * emit a single closed contour so `fill()` paints the bounded region.
 *
 * Members must match the names registered with `presetShapes.set(...)`
 * below. Closed list — no new members are expected.
 */
export const STROKE_ONLY_PRESETS: ReadonlySet<string> = new Set([
    'line',
    'lineInv',
    'straightConnector1',
    'bentConnector2',
...
presetShapes.set('bracketPair', (w, h, adjustments) => {
  // OOXML: adj=16667 (max 50000), radius = ss * a / 100000
    ...
    const x2 = w - r;
    const y2 = h - r;
  // Single closed contour so `fill()` paints the bounded interior:
  // start at left bracket's top-end → top edge across to right
  // bracket's top-end → trace right bracket down → bottom edge back
  // to left bracket's bottom-end → trace left bracket up → close.
    return [
        `M${r},0`,
        `A${r},${r} 0 0,0 0,${r}`,
        `L0,${y2}`,
...
presetShapes.set('bracePair', (w, h, adjustments) => {
    ...
    const r = Math.min(w, h) * a;
    const cy = h / 2;
  // Single closed contour (left brace + top edge across + right brace
  // mirrored + bottom edge back) so `fill()` paints the bounded
  // interior. Word renders bracePair as a filled rounded-rect-ish
  // body with the two brace silhouettes on the sides.
    return [
...
presetShapes.set('arc', (w, h, adjustments) => {
  ...
  // OOXML angles are "visual" (geometric) — must convert to parametric for ellipses (rx≠ry).
    ...
    if (sweepDeg === 0 && startDeg !== endDeg) sweepDeg = 360;
    const largeArc = sweepDeg > 180 ? 1 : 0;
  // OOXML `arc` renders as a pie wedge: arc from start→end joined back
  // to the centre by two radii, closed and filled. ECMA-376's path
  // definition emits an open arc; Word fills it as a wedge by treating
  // the implicit close as a chord-to-centre. We make that explicit so
  // Canvas's `fill()` matches Word instead of filling the chord-only
  // region (a flat segment of the disc).
...
// Magnetic disk / drum cylinder shapes — overrides for the single-path
// versions registered in `presetShapes`. Splitting the body from the
// front-edge detail line lets PresetGeometryRect render the body with
// fill+stroke and the detail line as stroke-only. With the original
// single-path flattening, the detail arc would participate in the
// evenodd fill and cut the cylinder body open into a half-moon void.
//
// Names use the lowercase form because multiPathPresets is queried
// via `key.toLowerCase()` (see getMultiPathPreset).
...
multiPathPresets.set('flowchartmagneticdrum', (w, h) => {
  // Horizontal cylinder. Caps on the left and right, body is the
...
const _warnedUnknownPresets = new Set<string>();
...
export function getPresetShapePath(
  ...
  // Fallback: simple rectangle. `getPresetShapePath` runs on every
  // repaint (Univer's render loop redraws drawings each frame), so
  // warning unconditionally floods DevTools and freezes it after a
  // few seconds when a fixture has many unsupported presets. Warn
  // once per preset name instead.
  //
  // Caller (PresetGeometryRect) tries `getMultiPathPreset` first and
  // only falls through here when neither registry has the preset, so
  // a warn here means truly unknown.
...
//
// The following generators are implemented here because they appear in
// our DOCX import fixture (`全格式.docx`) but were missing from upstream
// at the vendored revision (e6200e8). The geometry math is derived from
// the OOXML spec (ECMA-376 Part 1, §20.1.9.18 + Annex) via the public
// PPTXjs (MIT) port. The `<a:avLst>` adjust value is not yet plumbed
// through Univer's importer, so each generator uses the OOXML-defined
// default and ignores the `adjustments` argument for now.
// ─────────────────────────────────────────────────────────────────────────

presetShapes.set('smileyFace', (w, h) => {
  // Default `adj1` is 4653 (in 100000ths). Spec: -4653 ≤ adj1 ≤ 4653.
  // dy2 lifts the mouth corners up by adj/100000 of the height; dy3
  // shifts the mouth's quadratic control point further down by adj/50000
  // (so a positive adj makes a happier curve).
    const wd2 = w / 2;
    const hd2 = h / 2;
    const x1 = (w * 4969) / 21699;
    const x2 = (w * 6215) / 21600;
    ...
    const y3 = (h * 16515) / 21600;
    const adj = 4653;
    const dy2 = (h * adj) / 100000;
    const dy3 = (h * adj) / 50000;
    const y2 = y3 - dy2;
    const y4 = y3 + dy2;
    const y5 = y4 + dy3;
    const wR = (w * 1125) / 21600;
    const hR = (h * 1125) / 21600;
  // Eyes — full ellipses (sweep 180°→540° = 360°). Centre-x derived to
  // match OOXML's "shape arc starts at angle 180" convention.
    const cX1 = x2 + wR;
    const cX2 = x3 + wR;
    const cY1 = y1;
    return (
    ...
    // Mouth (closed quadratic curve, both directions).
    `M${x1},${y2} Q${wd2},${y5} ${x4},${y2} Q${wd2},${y5} ${x1},${y2} ` +
    // Head — full ellipse, drawn last so its outline closes the figure.
    `M0,${hd2} ${shapeArc(wd2, hd2, wd2, hd2, 180, 540, false).replace('M', 'L')} Z`
    );
});

/**
 * Action button outer frame + glyph. Both `home` and `information`
 * draw the OOXML "outer rectangle then a centred glyph" pattern; only
 * the glyph differs.
 */
function actionButtonFrame(w: number, h: number): string {
    return `M0,0 L${w},0 L${w},${h} L0,${h} Z`;
}

presetShapes.set('actionButtonHome', (w, h) => {
  // House glyph: roof triangle + base rectangle + door cutout. Magic
  // ratios from OOXML spec (3/4 the smaller side, partitioned in 16ths).
    const hc = w / 2;
    const vc = h / 2;
    const ss = Math.min(w, h);
    const dx2 = (ss * 3) / 8;
    ...
    const g33 = g11 + g23;
    return (
        `${actionButtonFrame(w, h)} ` +
    // House outline.
    `M${hc},${g9} L${g11},${vc} L${g28},${vc} L${g28},${g10} L${g33},${g10} ` +
    `L${g33},${vc} L${g12},${vc} L${g32},${g26} L${g32},${g24} L${g31},${g24} L${g31},${g25} Z ` +
    // Door cutout.
...
presetShapes.set('actionButtonInformation', (w, h) => {
  // Lower-case "i" glyph: head dot (small filled circle), stem (rounded
  // rectangle), drawn inside an outer frame. Magic ratios per spec.
    const hc = w / 2;
    const vc = h / 2;
    const ss = Math.min(w, h);
    const dx2 = (ss * 3) / 8;
    ...
    return (
        `${actionButtonFrame(w, h)} ` +
    // Outer "i" head ring (full ellipse).
    `M${hc},${g9} ${shapeArc(hc, cY1, dx2, dx2, 270, 630, false).replace('M', 'L')} Z ` +
    // Inner head dot.
    `M${hc},${g25} ${shapeArc(hc, cY2, g38, g38, 270, 630, false).replace('M', 'L')} Z ` +
    // Stem (12-vertex polygon).
    `M${g32},${g28} L${g35},${g28} L${g35},${g30} L${g37},${g30} L${g37},${g31} ` +
    `L${g32},${g31} L${g32},${g30} L${g34},${g30} L${g34},${g29} L${g32},${g29} Z`
    );
});
