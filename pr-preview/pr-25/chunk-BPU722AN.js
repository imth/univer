...
// ../packages/docs-drawing-ui/src/controllers/text-box-edit.controller.ts
var TextBoxEditController = class extends Disposable {
  constructor(_instanceSrv, _renderManagerService, _drawingManagerService) {
    super();
    __publicField(this, "_renderManagerService", _renderManagerService);
    __publicField(this, "_drawingManagerService", _drawingManagerService);
  ...
  _findSearchById(unitId, drawingId) {
    ...
    if (!group) return null;
    for (const subUnitId in group) {
      const drawings = (_b = (_a = group[subUnitId]) == null ? void 0 : _a.data) != null ? _b : {};
  ...
  _exitEdit() {
    ...
    const render2 = this._renderManagerService.getRenderById(search.unitId);
    if (!render2) return;
    this._applyEditState(render2, search.drawingId, false);
  }
  _applyEditState(render2, drawingId, on) {
    const viewModel = render2.with(DocSkeletonManagerService).getViewModel();
    const docSelectionRenderService = render2.with(DocSelectionRenderService);
    viewModel.setEditArea(on ? "TEXT_BOX" /* TEXT_BOX */ : "BODY" /* BODY */);
    docSelectionRenderService.setSegment(on ? drawingId : "");
    const search = this._findSearchById(render2.unitId, drawingId);
    if (!search) return;
    const shapeKey = getDrawingShapeKeyByDrawingSearch(search);
    const overlay = render2.scene.getObject(`${shapeKey}${SHAPE_TEXT_OVERLAY_SUFFIX}`);
  ...
  _wireExitListeners() {
    ...
    if (!this._activeSegment) return;
    const render2 = this._renderManagerService.getRenderById(this._activeSegment.unitId);
    if (!render2) return;
    const activeShapeKey = getDrawingShapeKeyByDrawingSearch(this._activeSegment);
    this._outsideClickSub = render2.scene.onPointerDown$.subscribeEvent((evt) => {
      const target = evt.target;
      if ((target == null ? void 0 : target.oKey) === activeShapeKey) return;
      if ((target == null ? void 0 : target.oKey) === `${activeShapeKey}${SHAPE_TEXT_OVERLAY_SUFFIX}`) return;
      this._exitEdit();
    });
  }
  _unwireExitListeners() {
...
TextBoxEditController = __decorateClass([
