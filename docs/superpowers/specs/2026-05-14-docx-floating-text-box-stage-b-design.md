...
| ~~**Double-click to edit**~~ | **Deferred to a separate PR.** Spike found that `engine-render`'s `RichText` is a read-only render component (no `editingEnabled` switch); the real editor entry, `IEditorService.register`, requires an `HTMLDivElement` container — i.e. a DOM overlay positioned over the canvas. That pulls in the same plumbing as full toolbar-routed editing (E2), so splitting "minimal text-only edit" out separately would leave a half-finished UX. Will land as Stage C with full toolbar routing. |
